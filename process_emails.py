...
from models.email import FilterEmailsRequest, EmailColumnName
...
class RulesProcessor:
    ...
    def transform_to_db_request(self, rule_collection: EmailRulesConfig.EmailRulesCollection) -> FilterEmailsRequest:
        ...
        db_collection_predicate = collection_predicate_map.get(rule_collection.predicate)
        if db_collection_predicate == None:
            raise ValueError(f"Invalid Rule Predicate value: {rule_collection.predicate}")
        ...
        return FilterEmailsRequest(filter=FilterEmailsRequest.RulesCollection(
            predicate=db_collection_predicate,
            rules=db_rules
        ))
...
def run():
    ...
    for rule_collection in rules_processor.collections:
        ...
        email_manager.filter(filter_req)
